...
import { Award, Calendar, Building2, Eye, ExternalLink } from "lucide-react";
      ...
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="max-w-3xl">
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-border/50">
            {selected && isPdf(selected.certificateImage) ? (
              <object
                data={`${selected.certificateImage}#view=FitH&toolbar=0&navpanes=0`}
                type="application/pdf"
                className="h-[70vh] w-full"
                aria-label={`Certificado de ${selected.title}`}
              >
